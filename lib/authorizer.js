...
Authorizer.prototype.authorize = function (credentials, cb) {
  ...
  console.log('bitbucket authorizer: ', scope, token, packagePath)
  ...
  // var handleBitbucketResponse = this._handleBitbucketResponse(fetchRepoInfo)
  // return handleBitbucketResponse.nodeify(cb)
        ...
        return Promise.resolve(getSession).then(function (session) {
          return Promise.resolve(session.getRefreshToken(token)).then(function (refreshToken) {
            ...
            .then(function (auth) {
              ...
              // return Promise.resolve(privilegesRequest.accessToken(auth.token).get())
            })
...
Authorizer.prototype._getSession = function () {
  ...
  return Promise.try(function () {
    if (!self.session) {
      console.log('bitbucket authorizer: creating session')
      self.session = new Session(self.opts)
    }
    return self.session
  })
}
