...
Authenticator.prototype._unauthenticate = function (token, deleteUserData) {
  // TODO this method should delete alias and referenced user/refresh keys too?
...
Authenticator.prototype._validateCredentials = function (credentials) {
  return Boolean(credentials && credentials.body && (credentials.body.email || credentials.body.name) && credentials.body.password)
}

Authenticator.prototype._getAuthorizationToken = function (credentialsBody) {
  var self = this
  credentialsBody = credentialsBody || {}
  var email = credentialsBody.email || credentialsBody.name || ''
